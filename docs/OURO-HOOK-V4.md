# OuroHookV4 — o motor de lucro do Ouroboros em v4 (Fase A)

Primeira peça da migração v3 → v4 do Ouroboros (o plano completo está em
`OUROBOROS-V4-PLANO.md`). Aqui está o **núcleo do lucro**: um hook Uniswap v4 que cobra uma
...
Robinhood Chain real. A portabilidade pro repositório do Ouroboros (o `Launchpad` gravar este
hook no lugar do `FeeLocker`) é a **Fase B**.

## O que muda em relação ao v3

| v3 (hoje) | v4 (`OuroHookV4`) |
| --- | --- |
...
Swap  →  taxa 1% sobre o input (ETH nas compras, token nas vendas)  →  no hook:
    ...
    └─ 0,20%  BURN       → platformTreasury (buy&burn do OURO)
```
...
- `contracts/src/OuroHookV4.sol` — o hook (ERC-20 + LP owner + roteador de taxa).
- `contracts/test/OuroHookUnit.t.sol` — 15 testes de lógica (mocks v4): split, acumulador
...
- `contracts/test/e2e/OuroHookE2E.t.sol` — prova end-to-end contra PoolManager/POSM reais
  (swaps de verdade): a taxa é skimmada no swap, holders acumulam, protocolo saca.
- `contracts/test/e2e/OuroHookFork.t.sol` — ciclo completo contra a Robinhood Chain real.
- `contracts/script/DeployOuro.s.sol` — deploy com mineração CREATE2 pras flags
...
```bash
cd contracts
FOUNDRY_PROFILE=sandbox forge test --use ./solc-wrapper.js --match-contract OuroHookUnitTest -vv
```

**E2E contra um stack v4 local (precisa de solc nativo):**

```bash
FOUNDRY_PROFILE=e2e forge test --match-contract OuroHookE2ETest -vv
```
...
export PLATFORM_TREASURY=<tesouraria de buy&burn do OURO>
...
Fazer o `Launchpad.sol` do Ouroboros gravar o `OuroHookV4` na graduação (no lugar do
`FeeLocker`), minerando o endereço com o `HookMiner`. O front-end lê os novos campos de fee e
remove o botão de harvest (agora automático). Detalhes em `OUROBOROS-V4-PLANO.md`.
